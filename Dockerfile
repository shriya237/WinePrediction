...
#RUN mkdir WinePredict

ENV PROG_DIR /WinePredict/
ADD data/ValidationDataset.csv /WinePredict/

COPY trainingmodel.model/ trainingmodel.model/
ADD wineTest.py .

ADD data/ValidationDataset.csv .
RUN ls
#RUN hadoop fs -put /WinePredict/ValidationDataset.csv /ValidationDataset.csv

ENV PROG_NAME wineTest.py
ADD ${PROG_NAME} .

ENTRYPOINT ["spark-submit","wineTest.py"]
